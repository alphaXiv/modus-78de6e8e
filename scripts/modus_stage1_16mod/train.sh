#!/bin/bash
#SBATCH --job-name=modus-stage1-16mod
...
#SBATCH --output=logs/modus_stage1_16mod_%j.out
#SBATCH --error=logs/modus_stage1_16mod_%j.err
...
# header via `sbatch` CLI flags (e.g. `sbatch --nodes=8 --gpus-per-node=8
# --account=YOURACCT ...`) or by editing it. `--environment` is CSCS-pyxis-only;
# on other clusters drop it and launch inside your own container instead.
MODUS_ROOT="${MODUS_ROOT:-/path/to/your/MODUS}"

# 16-modality stage1 (13 old + cocodet; rgb2cocodet). Uses the cocodet-patched
# base ckpt (checkpoint_name in conf/train/modus_stage1_16mod.yaml). Loads the
# consolidated base ckpt via the auto full-state path (no sharded resume dir →
# no MODUS_FORCE_FULL_STATE_LOAD needed). Validated by smoke_1node.sh.
TRAIN_CONFIG="modus_stage1_16mod"

export output_path="${output_path:-${MODUS_ROOT}/results_paper/stage1/modus_16mod_stage1_cocodet}"
...
echo "Starting MODUS stage-1 16mod (cocodet) 16-node training..."
