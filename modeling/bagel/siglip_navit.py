...
from ._attn_fallback import flash_attn_varlen_func
...
class RotaryEmbedding2D(torch.nn.Module):
    ...
    def reset_parameters(self):
        # Re-run the buffer initialization logic
        freq = torch.arange(0, self.cos_h.shape[-1], 2, dtype=torch.int64).float() / self.cos_h.shape[-1]
        inv_freq = 1.0 / (10000 ** freq)
        max_h, max_w = self.cos_h.shape[0], self.cos_w.shape[1]
