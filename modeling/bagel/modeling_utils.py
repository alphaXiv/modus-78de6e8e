...
class LearnableEmbedding(nn.Module):
    def __init__(self, length, hidden_size):
        ...
        self.embedding = nn.Parameter(
            torch.zeros(length, hidden_size), 
            requires_grad=True
        )
        self._init_weights()

    def _init_weights(self):
        # Initialize learnable embedding with small random values
...
class PositionEmbedding(nn.Module):
    def __init__(self, max_num_patch_per_side, hidden_size):
        ...
        self.hidden_size = hidden_size
        # Fixed sin-cos positional embedding: keep as buffer so FSDP does not
        # shard/all-gather it as a parameter. This avoids collectives on a
        # module that can be called a variable number of times across ranks.
