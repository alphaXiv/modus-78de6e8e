...
# DEFAULT: use torchvision exactly as upstream — training and inference are
# unchanged. Only when MODUS_TORCHVISION_FREE=1 (or torchvision is not importable,
# e.g. the HF ZeroGPU image whose custom torch has no matching torchvision wheel)
# do we fall back to the pure-PyTorch/PIL equivalents below.
_TORCHVISION_FREE = os.environ.get("MODUS_TORCHVISION_FREE", "0") == "1"
if not _TORCHVISION_FREE:
    try:
        ...
        def _make_to_tensor():
            return _tv_transforms.ToTensor()

        def _make_normalize(mean, std):
            return _tv_transforms.Normalize(mean=mean, std=std, inplace=True)
    except Exception:  # torchvision missing -> use the fallback path
        _TORCHVISION_FREE = True

if _TORCHVISION_FREE:
    # ── Pure-PyTorch/PIL fallback (no torchvision). For PIL inputs this matches
    # torchvision (F.resize on a PIL image delegates to PIL.Image.resize;
    # ToTensor/Normalize are plain arithmetic). The tensor-input resize path is a
    # reimplementation via F.interpolate and is NOT bit-exact with torchvision. ──
    import torch.nn.functional as _F

    _PIL_RESAMPLE = {"bicubic": Image.BICUBIC, "bilinear": Image.BILINEAR, "nearest": Image.NEAREST}
    ...
    def _resize(img, size, interpolation="bicubic", antialias=True):
        new_h, new_w = size
        if isinstance(img, torch.Tensor):
            x = img if img.dim() == 4 else img.unsqueeze(0)
            aa = antialias and interpolation in ("bicubic", "bilinear")
            x = _F.interpolate(x.float(), size=(new_h, new_w), mode=interpolation,
                               align_corners=False if interpolation != "nearest" else None,
                               antialias=aa)
            return x if img.dim() == 4 else x.squeeze(0)
        return img.resize((new_w, new_h), resample=_PIL_RESAMPLE[interpolation])

    def _to_tensor(img):
        """PIL image -> C×H×W float tensor in [0, 1] (matches transforms.ToTensor)."""
        arr = np.asarray(img if img.mode == "RGB" else img.convert("RGB"), dtype=np.uint8)
        return torch.from_numpy(arr).permute(2, 0, 1).contiguous().float().div_(255.0)

    def _normalize(t, mean, std):
        m = torch.as_tensor(mean, dtype=t.dtype, device=t.device).view(-1, 1, 1)
        s = torch.as_tensor(std, dtype=t.dtype, device=t.device).view(-1, 1, 1)
    ...
    def _make_normalize(mean, std):
        return lambda t: _normalize(t, mean, std)
...
class MaxLongEdgeMinShortEdgeResize(torch.nn.Module):
    ...
    def forward(self, img, img_num=1):
        ...
        return _resize(img, (new_height, new_width), self.interpolation, antialias=self.antialias)
...
class ImageTransform:
    def __init__(
        ...
        self.to_tensor_transform = _make_to_tensor()
        self.normalize_transform = _make_normalize(image_mean, image_std)
